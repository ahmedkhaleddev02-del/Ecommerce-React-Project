...
export default function SpicificBrand() {
  ...
  let { id, brandname } = useParams();

  function getSpecificBrand(idParameter) {
    return axios
      .get(`https://ecommerce.routemisr.com/api/v1/brands/${idParameter}`)
      .then((res) => {
    ...
    axios
      ...
      .then((res) => {
        let related = res.data.data.filter(
          (productaye) => productaye.brand.name == brandname
        );
        // console.log(related);
  ...
  return (
    <>
      {/* Dispaly Related Products Of choosen Brand */}
